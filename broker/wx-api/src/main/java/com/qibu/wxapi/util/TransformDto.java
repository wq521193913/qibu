	...
	*/
	public static <T> List<T> listMapToEntity(List<Map<String, Object>> resourList, Class<T> tagClass) throws Exception{
		...
		for (Map<String, Object> resourMap : resourList){
			T tagT = TransformDto.mapToEntity(resourMap,tagClass);
			resultList.add(tagT);
		}
		return resultList;
	}
...
//				resultMap.put(fieldName, null != field.get(entity) ? field.get(entity) : "");
	...
	public static List<Map<String, Object>> getSpecifiedFieldMap(String fields,List<? extends Object> entitys) throws Exception{
		return TransformDto.getSpecifiedFieldMap(fields, entitys, null);
	}
		...
		if(null != entitys && entitys.size() >0){
			for(Object entity : entitys){
				...
				if(entity.getClass().getName().indexOf("Map") > 0){
					...
					resultMap = TransformDto.getSpecifiedFieldMap(fields, sourceMap);
				}else {
					resultMap = TransformDto.getSpecifiedFieldMap(fields, entity, dateFormatMap);
				}
	...
	public static <T> T JsonObjectToEntity(JSONObject source, Class<T> entityClass){
		...
		try {
			...
			Class tag = entityClass;

			for(;tag != Object.class;tag = tag.getSuperclass()){
				Field[] fields1 = tag.getDeclaredFields();
				fields = (Field[]) ArrayUtils.addAll(fields,fields1);
			}
			for (;set.iterator().hasNext();name = set.iterator().next().toString()){
				try {
	...
	public static void JsonObjectToEntity(JSONObject source, Object entity){
		...
		try {
			...
			for (;set.iterator().hasNext();name = set.iterator().next().toString()){
				try {
					field = entity.getClass().getField(name);
					if(null != field){
						field.setAccessible(true);
						field.set(entity, source.get(name));
						field.setAccessible(false);
						field = null;
					}
				}catch (Exception e){
